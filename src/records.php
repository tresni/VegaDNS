...
if(!isset($_REQUEST['record_mode']) || $_REQUEST['record_mode'] == 'delete_cancelled') {
    ...
    while (++$counter && $row = $stmt->fetch()) {
        ...
        if($counter <= $last_record) {
            ...
            $records[$counter]['location'] = get_location_name($row['location_id']);
        }
    }
    ...
    if($result != 'OK') {
        ...
        $params = array(':location' => isset($_REQUEST['location']) ? $_REQUEST['location'] : '');
        if($_REQUEST['type'] == 'A') {
            $params[':address'] = $_REQUEST['address'];
            $q = "insert into records
            (domain_id,host,type,val,ttl,location_id) values(
            '".get_dom_id($domain)."',
            '$name',
            ...
            $q = "insert into records
            (domain_id,host,type,val,distance,ttl,location_id) values(
            '".get_dom_id($domain)."',
            '$name',
            '".set_type($_REQUEST['type'])."',
            :mxaddress,
        ...
        if($_REQUEST['type'] == 'SRV') {
            ...
            $q = "insert into records
            (domain_id,host,type,val,distance,weight,port,ttl,location_id) values(
            '".get_dom_id($domain)."',
            '$name',
            '".set_type($_REQUEST['type'])."',
            :srvaddress,
    ...
    $smarty->assign('location', $row['location_id']);
